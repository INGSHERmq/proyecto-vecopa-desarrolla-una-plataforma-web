...
async function main() {
    const roles = await Promise.all(['administrador', 'cajero', 'mozo'].map((name) => prisma.role.upsert({
        where: { name },
        update: {},
        create: { name },
    })));
    ...
    const users = [
        { name: 'Valeria Ramos', email: 'admin@vecopa.pe', password: 'Admin123!', role: 'administrador' },
        { name: 'Caja Norte', email: 'caja@vecopa.pe', password: 'Caja123!', role: 'cajero' },
        { name: 'Mozo Patio', email: 'mozo@vecopa.pe', password: 'Mozo123!', role: 'mozo' },
    ];
    ...
    const tableData = [
        { name: 'Mesa 1', capacity: 2, status: 'ocupada' },
        { name: 'Mesa 2', capacity: 4, status: 'libre' },
        { name: 'Terraza 1', capacity: 4, status: 'reservada' },
        { name: 'Barra', capacity: 6, status: 'ocupada' },
        { name: 'Mesa 5', capacity: 2, status: 'libre' },
        { name: 'Salon 2', capacity: 8, status: 'libre' },
    ];
    for (const table of tableData) {
        const exists = await prisma.diningTable.findFirst({ where: { name: table.name } });
        if (!exists)
            await prisma.diningTable.create({ data: table });
    }
}
main()
    .then(async () => {
